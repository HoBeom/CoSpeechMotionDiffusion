...
def train_iter_diffusion_text(args, in_audio, target_poses, in_text_padded, pose_diffusion, optimizer):
    ...
    loss = pose_diffusion.get_loss(target_poses, pre_seq, in_audio, in_text_padded)
    loss.backward()
    clip_grad_norm_(pose_diffusion.parameters(), 10)
    optimizer.step()
